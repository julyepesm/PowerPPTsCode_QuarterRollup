...
The launcher opens the server on port 8502; visit `http://localhost:8502` and
keep its terminal window open. Power BI now uses your colleague's device-code
sign-in flow: follow the code/link printed in that terminal.
...
Original development branch: `fix/rollup-catalog`. Implementation commit:
`e6c1de3` (`[Logic/Calculation Change] Fix client-scoped rollups, full YTD, and GMC XTPC split`).
The handoff was added in documentation commit `a04c2d9`. The subsequent merge
into `main` also retains the colleague's commits `edc8820`, `8901001`, and
`6b65988`: device-code sign-in, catalog diagnostics, and visible launcher output.
The launcher conflict was resolved by retaining port 8502, terminal messages,
and pause behavior together with the local-venv selection and root working
directory. Live deck generation described above preceded the device-code
authentication change; that new sign-in flow has not been live-tested here.
